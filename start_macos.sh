...
# Mental Health Prediction System - macOS Startup Script
...
echo -e "======================================================${NC}"
echo "Starting macOS Setup and Launch Process..."
echo
...
if ! command -v python3 &> /dev/null; then
    ...
    echo "Recommended: Install Python via Homebrew"
    echo "Install Homebrew first: /bin/bash -c \"\$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)\""
    echo "Then: brew install python"
    echo ""
    echo "Alternative: Download from https://www.python.org/downloads/macos/"
    exit 1
fi
...
if ! python3 -m pip --version &> /dev/null; then
    print_error "pip not found. Please reinstall Python with pip."
    exit 1
fi
